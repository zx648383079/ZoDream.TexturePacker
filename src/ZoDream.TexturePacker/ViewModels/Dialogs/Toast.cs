...
namespace ZoDream.TexturePacker.ViewModels
{
    public class Toast
    {
        ...
        /// </summary>
        private void Show(string text, string imagePath)
        {
            ShowToastNotification(imagePath, text, NotificationAudioNames.Default);
        }

        public void ShowToastNotification(string assetsImageFileName, string text, NotificationAudioNames audioName)
        {
            // 1. create element
            ...
            // 3. provide image
            if (!string.IsNullOrWhiteSpace(assetsImageFileName))
            {
